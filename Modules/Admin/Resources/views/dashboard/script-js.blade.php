  ...
  $(function(){
      ...
      $.ajax({
        ...
        success: function(response){
          ...
          if(response.is_published == 1 && response.is_deleted != 1){
            ...
            $(document).find('.draft-post-link').attr('href', '/admin/dashboard/' + response.id + '/make-draft');
          ...
          if(response.is_published != 1 && response.is_deleted != 1){
            ...
            $(document).find('.publish-post-link').attr('href', '/admin/dashboard/' + response.id + '/publish');
          ...
          if(response.is_deleted == 1){
            ...
            // Show restore button
            $(document).find('.restore-post-link').attr('href', '/admin/dashboard/' + response.id + '/restore');
    ...
    $(document).on('click', '#btnDeleteMultiple', function(){
      ...
      if(confirm("Are you sure you want to delete these posts?")){
        ...
        $.ajax({
          url: "{{ route('admin.dashboard.delete.multiple') }}",
          dataType: 'json',
          type: 'post',
          data: {
            post_ids: postIds
