<aside class="sidebar sidebar--static@md col-3@md js-sidebar sidebar--right-on-mobile" data-static-class="sidebar--sticky-on-desktop z-index-1 bg-contrast-lowest" id="sidebar" aria-labelledby="sidebarTitle">
  ...
  <nav class="sidenav padding-y-sm text-sm@md js-sidenav position-fixed">
    <div class="sidenav__label margin-bottom-xxxs">
      <span class="text-sm color-contrast-medium">Gifs</span>
    </div>

    <ul class="sidenav__list">

      <li class="sidenav__item sidenav__item--expanded">
        <a href="{{ url('/admin/gifs') }}" data-tab="published" class="sidenav__link ajax-link"{{ (url('/admin/gifs') == url()->full()) ? 'aria-current=page' : '' }}>
          ...
          <span class="sidenav__text">All Gifs</span>
          <span class="sidenav__counter">{{ $gifs_published_count }} <i class="sr-only">notifications</i></span>
        </a>
    ...
    <ul class="sidenav__list">
      <li class="sidenav__item">
        <a href="{{ url('/admin/gifs/settings') }}" class="sidenav__link"{{ (url('/admin/gifs/settings') == url()->full()) ? 'aria-current=page' : '' }}>
