...
<script>
  (function(){
    ...
    $(document).on('click', '.ajax-link', function (e) {
      ...
      localStorage.setItem("cs_user_gifs_init_tab", $(this).attr('data-tab'));

      // loads page content inside this element
      $('#site-table-with-pagination-container').load(url, function(){
        $('.add-gif-wrp').addClass('hidden'); // hide add gif section
        $('.gifs-wrp').removeClass('hidden'); // show gif list section
        $('.edit-gif-wrp').addClass('hidden'); // hide gif edit section
    ...
    var init_tab = localStorage.getItem("cs_user_gifs_init_tab");
    if (init_tab != null && document.referrer == document.location) {
      $('[data-tab="' + init_tab + '"]').trigger('click');
    } else {
      localStorage.setItem("cs_user_gifs_init_tab", ""); // clear
    }
  })();
</script>
...
<script>
  ...
  $(function(){
    ...
    // add new gif link
    $(document).on('click', '.btn-new-gif', function() {
      clearError($('#formAddGif'));

      $('.add-gif-wrp').removeClass('hidden'); // show add gif section
      $('.gifs-wrp').addClass('hidden'); // hide gif list section
      $('.edit-gif-wrp').addClass('hidden'); // hide gif edit section

      $('.subnav a[aria-current]').attr('aria-current', '');
    });

    // cancel gif add/edit link
    $(document).on('click', '.btn-cancel-gif', function() {
      $('.add-gif-wrp').addClass('hidden'); // hide add gif section
      $('.gifs-wrp').removeClass('hidden'); // show gif list section
      $('.edit-gif-wrp').addClass('hidden'); // hide gif edit section
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if(response.is_published == 1 && response.is_deleted != 1){
            ...
            $(document).find('.draft-gif-link').attr('href', '/gifs/' + response.id + '/make-draft');
          ...
          if(response.is_published != 1 && response.is_deleted != 1){
            ...
            $(document).find('.publish-gif-link').attr('href', '/gifs/' + response.id + '/publish');
          ...
          if(response.is_deleted == 1){
            ...
            // Show restore button
            $(document).find('.restore-gif-link').attr('href', '/gifs/' + response.id + '/restore');
            $(document).find('.restore-gif-link').removeClass('is-hidden');
          }
          ...
          clearError($('#formEditGif'));

          $('.add-gif-wrp').addClass('hidden'); // hide add gif section
          $('.gifs-wrp').addClass('hidden'); // hide gif list section
          $('.edit-gif-wrp').removeClass('hidden'); // show gif edit section
