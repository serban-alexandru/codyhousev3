<div class="bg radius-md padding-md shadow-sm">
  @if(request()->has('status') && request('status') == 'deleted' && auth()->user()->isAdmin())
    <div class="margin-bottom-md">
      <form action="{{ route('gifs.trash.empty') }}" method="post">
        @csrf
  ...
  <form action="{{route('gifs.delete.multiple')}}"
          ...
          @foreach($gifs as $gif)
            <tr class="int-table__row">
              ...
              <td class="int-table__cell text-center cursor-pointer">
                @if(is_null($gif->thumbnail_medium))
                  <a href="
                    {{
                        route(
                            'single-gif-view',
                ...
                " target="_blank" class="post-table-image-wrapper post-table-image bg-black bg-opacity-50%">
                  <img src="{{ $gif->showThumbnail('medium', 'gif') }}">
                </a>
                @endif
              </td>

              <td class="int-table__cell cursor-pointer" aria-controls="modal-edit-gif" data-id="{{ $gif->id }}">
                <a href="#0">
                  {{ Str::limit($gif->title, 47) }}
                </a>
              </td>
              ...
              @if($gif->status != 'deleted')
                <td class="int-table__cell text-center flex" style="overflow: unset;">
                  <form action="{{ route('gifs.delete') }}" method="post">
                    @csrf
                  ...
                  @if($gif->status == 'published')
                    <a href="{{ route('gifs.make-draft', ['id' => $gif->id]) }}">
              ...
              @if($gif->status != 'published' && $gif->status != 'deleted' )
                ...
                <td>
                  <a href="{{ route('gifs.publish', ['id' => $gif->id]) }}" class="btn">Publish</a>
                </td>
                @endif
                @if( request()->has('status') && request('status') == 'pending' && auth()->user()->isAdmin())
              ...
              @elseif($gif->status == 'deleted')
                <td class="int-table__cell" style="overflow: unset;">
                  <a href="{{ route('gifs.restore', ['id' => $gif->id]) }}" class="btn margin-right-sm">Restore</a>
                </td>
                @if (auth()->user()->isAdmin())
                <td class="int-table__cell text-center" style="overflow: unset;">
                  <form action="{{ route('gifs.delete-permanently') }}" method="post">
                    @csrf
                    <input type="hidden" name="gif_id" value="{{ $gif->id }}">
  ...
  <div class="flex items-center justify-between padding-top-sm">
    ...
    <nav class="table-pagination-bottom pagination text-sm" aria-label="Pagination">
      <ul class="pagination__list flex flex-wrap gap-xxxs">
        <li>
          <a
            href="{{ $gifs->withQueryString()->previousPageUrl() }}"
            class="pagination__item
              {{ ($gifs->currentPage() == 1) ? 'pagination__item--disabled' : '' }}
            "
          >
        ...
        <li>
          <a
            href="{{ $gifs->withQueryString()->nextPageUrl() }}"
            class="pagination__item
              {{ !$gifs->hasMorePages() ? 'pagination__item--disabled' : '' }}
            "
          >
            <svg class="icon" viewBox="0 0 16 16">
...
<div class="bg radius-md padding-md margin-top-lg shadow-sm">
        ...
        <tbody class="int-table__body js-int-table__body">
          @foreach($rejected_gifs as $gif)
            <tr class="int-table__row">
              ...
              <td class="int-table__cell">{{ $gif->reject_reason }}</td>
  ...
  <div class="flex items-center justify-between padding-top-sm">
    <p class="text-sm">
      {{ $rejected_gifs->count() }}
      {{ ($rejected_gifs->count() < 2) ? 'result' : 'results' }}
    </p>

    @if($rejected_gifs->count() > 0)
    <nav class="table-pagination-bottom pagination text-sm" aria-label="Pagination">
      <ul class="pagination__list flex flex-wrap gap-xxxs">
        <li>
          <a
            href="{{ $rejected_gifs->withQueryString()->previousPageUrl() }}"
            class="pagination__item
              {{ ($rejected_gifs->currentPage() == 1) ? 'pagination__item--disabled' : '' }}
            "
          >
            <svg class="icon" viewBox="0 0 16 16">
              <title>Go to previous page</title>
        ...
        <li>
          <a
            href="{{ $rejected_gifs->withQueryString()->nextPageUrl() }}"
            class="pagination__item
              {{ !$rejected_gifs->hasMorePages() ? 'pagination__item--disabled' : '' }}
            "
          >
            <svg class="icon" viewBox="0 0 16 16">
              <title>Go to next page</title>
