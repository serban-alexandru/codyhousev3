              ...
              <div class="alert js-alert margin-bottom-lg alert--is-visible" role="alert"> <!-- /.alert--is-visible -->
                <div class="flex items-center justify-between">
                  <div class="flex items-center">
                    ...
                    <div class="alert-message">{{ session()->get('pages-settings-alert') }}</div><!-- /.alert-message -->
                  </div>
                </div>
              </div>
            @endif

            <form action="{{ (is_null($pages_settings)) ? route('pages.settings.store') : route('pages.settings.update') }}" method="post">
                @csrf
                <fieldset class="margin-bottom-md">
                  ...
                  <div class="grid gap-sm">
                    <div class="col-6@md">
                      <input class="form-control width-100%" type="text" value="{{ old('medium_width') ?: ((is_null($pages_settings)) ? '' : $pages_settings->medium_width) }}" name="medium_width" id="mediumWidth" placeholder="Enter width" required>
                    </div>
                
                    <div class="col-6@md">
                      <input class="form-control width-100%" type="text" value="{{ old('medium_height') ?: ((is_null($pages_settings)) ? '' : $pages_settings->medium_height) }}" name="medium_height" id="mediumHeight" placeholder="Enter height" required>
                    </div>
                </fieldset>
                ...
                <fieldset class="margin-top-xxl">
                  ...
                  <div class="flex flex-wrap gap-md">
                    @foreach(['maintain' => 'Maintain Aspect Ratio', 'crop' => 'Crop'] as $key => $image_setting)
                      <div>
                        <input class="radio" type="radio" value="{{ $key }}" name="image_setting" id="{{ $key }}"{{ (old('image_setting') == $key) ? ' checked' : ((!is_null($pages_settings) && $pages_settings->image_setting == $key) ? ' checked' : ((is_null($pages_settings) && $key == 'maintain') ? ' checked' : '')) }}>
                ...
                <fieldset class="margin-top-xl">
                  <h3 class="margin-bottom-sm">Listing per page</h3>
                    <div class="col-6@md">
                      <input class="form-control width-100%" type="text" value="{{ old('list_per_page') ? old('list_per_page') : ((!is_null($pages_settings)) ? $pages_settings->list_per_page : '') }}" name="list_per_page" id="listPerPage" placeholder="Enter amount">
                    </div>
                </fieldset>
                --}}
