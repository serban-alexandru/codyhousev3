<div class="bg-contrast-lower">
  <div class="container max-width-lg flex items-center justify-between">
    <div class="flex flex-wrap fl-align-center">
      <button class="btn btn--primary btn-new-page margin-right-md">Add Page</button>
            
      <form class="expandable-search js-expandable-search" action="{{ url('pages') }}" method="GET">
        <label class="sr-only" for="expandable-search">Search</label>
        <input class="reset expandable-search__input js-expandable-search__input" type="search" name="pagesearch" id="expandable-search" placeholder="Search..." required>
      ...
      <div class="subnav__wrapper js-subnav__wrapper">
        <nav class="subnav__nav ">
          ...
          <ul class="subnav__list">
            <li class="subnav__item"><a href="{{ url('/pages') }}" data-tab="published" class="subnav__link ajax-link" {{ (url('/pages') == url()->full()) ? 'aria-current=page' : '' }}>Published<span class="padding-left-sm sidenav__counter">{{ $pages_published_count }} <i class="sr-only">notifications</i></span></a></li>
            <li class="subnav__item"><a href="{{ url('/pages?is_draft=1') }}" data-tab="draft" class="subnav__link ajax-link">Draft<span class="padding-left-sm sidenav__counter">{{ $pages_draft_count }} <i class="sr-only">notifications</i></span></a></li>
            @if (!auth()->user()->isEditor())
            <li class="subnav__item"><a href="{{ url('/pages?is_pending=1') }}" data-tab="pending" class="subnav__link ajax-link">Pending<span class="padding-left-sm sidenav__counter">{{ $pages_pending_count }} <i class="sr-only">notifications</i></span></a></li>
            @endif
            <li class="subnav__item"><a href="{{ url('/pages?is_trashed=1') }}" data-tab="trashed" class="subnav__link ajax-link">Trash<span class="padding-left-sm sidenav__counter">{{ $pages_deleted_count }} <i class="sr-only">notifications</i></span></a></li>
          </ul>
        </nav>
      </div>
    </div> <!-- end of .subnav -->
