<div class="bg radius-md padding-md shadow-sm">
  @if(request()->has('is_trashed') && auth()->user()->isAdmin())
    <div class="margin-bottom-md">
      <form action="{{ route('pages.trash.empty') }}" method="post">
        @csrf
  ...
  <form action="{{route('pages.delete.multiple')}}"
          ...
          @foreach($pages as $page)
            <tr class="int-table__row">
              ...
              @if(!$page->is_deleted || ($page->is_published && !$page->is_deleted))
                <td class="int-table__cell text-center flex" style="overflow: unset;">
                  ...
                  @if($page->is_published && !$page->is_deleted)
                    <a href="{{ route('pages.make-draft', ['id' => $page->id]) }}">
              ...
              @if(!$page->is_published && !$page->is_deleted )
                ...
                <td>
                  <a href="{{ route('pages.publish', ['id' => $page->id]) }}" class="btn">Publish</a>
                </td>
                @endif
              ...
              @elseif($page->is_deleted)
                <td class="int-table__cell" style="overflow: unset;">
                  <a href="{{ route('pages.restore', ['id' => $page->id]) }}" class="btn margin-right-sm">Restore</a>
                </td>
                @if (auth()->user()->isAdmin())
                <td class="int-table__cell text-center" style="overflow: unset;">
                  <form action="{{ route('pages.delete-permanently') }}" method="post">
                    @csrf
                    <input type="hidden" name="page_id" value="{{ $page->id }}">
