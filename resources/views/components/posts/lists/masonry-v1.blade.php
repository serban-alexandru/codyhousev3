<div class="masonry js-masonry js-infinite-scroll container max-width-lg margin-top-md" data-path="{{ url('/api/posts/page/{n}') }}" data-container=".js-infinite-scroll__content" data-current-page="1">
  ...
  @foreach($posts as $post)
    ...
    @if($post->thumbnail)
      <img class="block width-500%" src="{{ $post->showThumbnail('original') }}" alt="Image of {{ $post->title }}">
    @else
      ...
      <div class="user-cell">
        <div class="user-cell__body">
          ...
          <div class="user-cell__content text-component line-height-sm v-space-xxs">
            <p><a class="color-contrast-high" href="#0"><strong>{{ $post->username }}</strong></a></p>
